...
const Dashboard = () => {
  ...
  const filterElections = (elections) => {
    if (tab === "all") {
      // Sort elections: Ongoing, In Process, Ended
      return elections.sort((a, b) => {
        const statusA = getStatus(a.start_datetime, a.end_datetime);
        const statusB = getStatus(b.start_datetime, b.end_datetime);

        // Priority: Ongoing > In Process > Ended
        if (statusA === "Ongoing" && statusB !== "Ongoing") return -1;
        if (statusA !== "Ongoing" && statusB === "Ongoing") return 1;
        if (statusA === "In Process" && statusB !== "In Process") return -1;
        if (statusA !== "In Process" && statusB === "In Process") return 1;
        return 0;
      });
    }
